      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="mb-8 animate-fade-in">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-12 h-12 bg-slate-800/60 border border-slate-700 rounded-xl flex items-center justify-center text-2xl">🏛️</div>
            <div>
              <h1 className="font-display text-5xl font-bold text-white">HOD Dashboard</h1>
          ...
          <div className="mt-3 inline-flex items-center gap-3 bg-slate-800/60 border border-slate-700 rounded-lg px-4 py-2">
            <span className="text-slate-300 text-sm font-medium">Review teacher-approved requests & request leave to Principal</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
          ...
          <div className="glass-card p-4 bg-slate-700/20">
            <p className="text-slate-500 text-sm font-medium mb-1">Approved (Final)</p>
            <p className="text-4xl font-bold text-emerald-300">{all.filter(l => l.status === 'approved').length}</p>
          </div>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${
                tab === item.key ? 'bg-slate-600 text-white shadow-lg shadow-slate-500/25' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              {item.label}
        ...
        {loading ? (
          ...
          <div className="glass-card p-6">
            <h2 className="text-white font-semibold text-lg mb-5">Request Leave to Principal</h2>
